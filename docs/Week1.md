...
- ディレクトリの初期化
  ...
  - packages/domain: ドメインロジック（住所正規化、入力検証、ビジネスルール）と Zod スキーマ群。
...
手順
1) Red（失敗するテストの先行）
   - packages/domain に住所リスト用スキーマのテストを作成（正常系/空行/2件未満/重複/トリム）。
   - apps/web にフォームの UI テスト（Testing Library）を用意：入力→エラー表示の確認。
   - Red であることの確認手順
    - `pnpm --filter @route-kun/domain test` を実行し、住所リスト用スキーマの 5 ケース（正常系/空行/2件未満/重複/トリム）がすべて失敗していることを確認。
    - `pnpm --filter web test` を実行し、UI テストが textarea/ボタン未実装のために失敗していることを確認。
2) Green（最小実装）
...
- TTL: 24 時間（MVP）、期限切れはバックグラウンド再取得で更新。将来的に 7〜30 日への延長を検討。
...
成果物
- docs/ に距離キャッシュ設計ドキュメント（例: docs/DistanceCacheDesign.md）を作成（キー、TTL、スキーマ、RLS、観測性、失敗時動作）。
...
- Day 6-7: バッファ（予期せぬ問題対応、CI/CD の初期設定、ドキュメント仕上げ、Week 2 準備）。
